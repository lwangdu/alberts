...
/**
 * Add accessible labels to FooBox lightbox controls generated by the plugin.
 */
function alberts_foobox_accessibility_labels() {
	?>
	<script>
		( function () {
			function labelFooBoxButtons() {
				var labels = {
					'.fbx-prev': 'Previous image',
					'.fbx-next': 'Next image',
					'.fbx-close': 'Close lightbox'
				};

				Object.keys( labels ).forEach( function ( selector ) {
					document.querySelectorAll( selector ).forEach( function ( button ) {
						if ( ! button.getAttribute( 'aria-label' ) ) {
							button.setAttribute( 'aria-label', labels[ selector ] );
						}

						if ( 'BUTTON' === button.tagName && ! button.getAttribute( 'type' ) ) {
			...
			new MutationObserver( labelFooBoxButtons ).observe( document.body, {
				childList: true,
				subtree: true
			} );
		}() );
...
add_action( 'wp_footer', 'alberts_foobox_accessibility_labels', 20 );
